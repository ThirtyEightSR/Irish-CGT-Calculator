...
def parse_type(desc: any) -> str:
    ...
    if "merger" in d or "merged" in d:
        return "Merger"
    if "return of capital" in d or "roc" in d:
    ...
    # NEW: return of capital (RoC)
    if "return of capital" in d or d.startswith("roc") or "capital repayment" in d:
        return "Return of capital"
...
def build_output(df_norm: pd.DataFrame, opening_lots: Optional[pd.DataFrame]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    base = pd.DataFrame({
        ...
        "FXCCY": df["__fx_ccy"],
        "__row_id": df["__row_id"],          # ← add this line
    })
