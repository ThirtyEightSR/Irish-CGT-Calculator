...
def parse_type(desc: any) -> str:
    # Fix 1: Ensure 're' is available if you encounter NameError. (Assuming top-level import is used)
    import re
    ...
    # --- 1. Corporate Actions & Special Disposals (Order matters) ---
    
    # Fix for TWTR Delisting: Treat Cash Settlement & Delisting as a Sell/Disposal
    # ... (around the corporate actions section)
    
    # Fix for TWTR Delisting: Classify non-cash delisting separate from Cash Settlement
    if "corporate action cash settlement" in d:
        return "Sell" # The line with the actual cash proceeds
    if "delisting" in d:
        return "Delisting" # New type: neutralized as a corporate action below
    ...
    if "spin-off" in d or "spinoff" in d or "demerger" in d:
        return "Spin-off"
    ...
    if "scrip dividend" in d or "stock dividend" in d:
...
def _carry_forward_shares_to_year(out_df: pd.DataFrame, year: int, use_exemption: bool, exemption_val: float) -> float:
    ...
    for yr in sorted(k for k in y.keys() if pd.notna(k) and k < year):
        ...
        if realised >= 0:
            ...
            taxable = max(0.0, remaining_gain - ex_used)
...
def _tax_shares_delta(ytd_gain_now: float, hypo_gain: float, carry_in: float, use_exemption: bool, exemption_val: float, rate: float) -> tuple[float, float, float]:
    """
...
def _replay_fifo_lots_all(out_df: pd.DataFrame) -> Dict[str, List[Dict]]:
    """
    FIFO replay for ALL trades in `out_df`.
    Returns: { ISIN: [ { "acq": Timestamp, "qty": float, "unit_cost_eur": float, "unit_cost_native": float }, ... ] }
    """
    ...
    # Helper to get float or 0.0
    def _f(val):
        try: return float(val)
        except: return 0.0

    # Ensure we have unit cost in EUR
    if "Price_EUR" not in df.columns:
        # Calculate it on the fly if missing
        tot = pd.to_numeric(df.get("Total (EUR, fee-adj)", df.get("Total (EUR)", np.nan)), errors="coerce").abs()
        qty = pd.to_numeric(df.get("Quantity"), errors="coerce").abs()
        df["__unit_cost"] = np.where(qty > 0, tot / qty, 0.0)
    else:
        df["__unit_cost"] = pd.to_numeric(df["Price_EUR"], errors="coerce").fillna(0.0)
    ...
    for isin, g in df.groupby("ISIN", sort=False):
        ...
        for _, r in g.iterrows():
            ...
            if t == "Buy":
                uc_eur = _f(r.get("__unit_cost"))
                
                # --- MATH FIX HERE ---
                # Calculate Native Unit Cost (e.g. USD) using the FX rate
                fx = _f(r.get("FX_Rate"))
                unit_native = np.nan
                
                if fx > 0:
                    # Multiply EUR cost by FX rate (EUR -> USD)
...
def _apply_corporate_actions_and_map_fx(base: pd.DataFrame, grouped: pd.DataFrame, opening_lots: Optional[pd.DataFrame]) -> Tuple[pd.DataFrame, List[Dict]]:
    ...
    def _agg_type(series: pd.Series) -> str:
        priorities = ["Buy","Sell","Dividend","Dividend Tax","Stock split","Product change","ISIN change","Delisting","Other"]
        types = [x for x in series.dropna().tolist() if isinstance(x, str)]
        for tpe in priorities:
            if tpe in types: return tpe
        return types[0] if types else "Other"

    # 1. Create the Grouped DataFrame (Aggregation)
    ...
    grouped["TradeCCY"] = grouped["__ccy_desc"].astype(str).str.upper().replace({"NONE": "", "NAN": "", "": np.nan}) if "__ccy_desc" in grouped.columns else np.nan

    # --- 2. STOCK SPLIT DETECTION AND ADJUSTMENT (Fixes Shopify) ---
    split_events = []
    split_audit = []

    # Detect splits from raw data (base)
    splits_raw = base[base["__Type"].eq("Stock split")].copy()

    for _, r in splits_raw.iterrows():
        desc = str(r.get("Description","")).lower()
        m = re.search(r"(\d+(?:\.\d+)?)[:\s]+(?:for|to)\s+(\d+(?:\.\d+)?)", desc)
        if m:
            a, b = float(m.group(1)), float(m.group(2))
            if b > 0: split_events.append((str(r["ISIN"]), pd.to_datetime(r["Date"]), a/b))
    
    split_events = sorted(list(set(split_events)), key=lambda x: x[1])

    # Apply Splits to Historical Buys
    if split_events:
        for isin, date, factor in split_events:
            mask = (grouped["ISIN"] == isin) & (grouped["Date"] < date) & (grouped["Type"] == "Buy")
            if mask.any():
                grouped.loc[mask, "__qty_desc"] *= factor
                grouped.loc[mask, "__price_desc"] /= factor
                
                for idx in grouped.index[mask]:
                    split_audit.append({
                        "ISIN": isin, "Split date": date, "Factor": factor,
                        "Row kind": "Buy (pre-split)", "Trade date": grouped.at[idx, "Date"],
                        "Qty (after)": grouped.at[idx, "__qty_desc"]
                    })

    # --- 3. CALCULATE SIGNED QUANTITY (uses adjusted __qty_desc) ---
    def _qty_signed(row):
    ...
    # --- 4. NEUTRALIZE CORPORATE ACTIONS (Fixes Product Change & Delisting rows) ---
    is_corp = grouped["Type"].isin(["Stock split","Product change","ISIN change", "Delisting"])
    if is_corp.any():
        # Set all numerical fields to NaN for these rows so they are ignored by FIFO/Tax calculations
        grouped.loc[is_corp, ["Quantity_signed", "__price_desc", "Fee_signed", "Total_signed", "Total_EUR", "Total_EUR_FeeAdj"]] = np.nan
        
    # --- 5. FEE & FX MAPPING (Original logic restored) ---
    # Fee aggregation (Original logic preserved)
    ...
    # Cash ex fees & unit price (Original logic preserved)
    ...
    # NEW: Ensure Total_signed is set correctly for all rows, especially Delistings
    if "Total_signed" not in grouped.columns:
        grouped["Total_signed"] = grouped["_CashValue"] # Uses aggregated cash change
    ...
    # --- NEW: Extract FX Rate by Order ID (to backfill trades) ---
    # This allows us to calculate the Native Cost (USD/GBP) for the BEP view
    fx_rate_map = (
        base.loc[base["FX_Rate"].notna() & base["Order ID"].ne(""), ["Order ID", "FX_Rate"]]
            .groupby("Order ID")["FX_Rate"]
            .max()  # Take the rate found on the FX line
            .rename("__fx_rate_by_oid")
            .reset_index()
    )
    eur_debit_by_oid = (
    ...
    # --- NEW: Merge FX Rate map and backfill ---
    grouped = grouped.merge(fx_rate_map, on="Order ID", how="left")
    # If the Trade row has no rate (common in DEGIRO), use the one found via Order ID
    grouped["FX_Rate"] = grouped["FX_Rate"].fillna(grouped["__fx_rate_by_oid"])
    ...
    try:
        is_incoming = (
            grouped["Type"].eq("Buy")
            & grouped["Description"].astype(str).str.contains("INCOMING TRANSFER", case=False, na=False)
        )
    ...
    is_corp = grouped["Type"].isin(["Stock split","Product change","ISIN change", "Delisting"]) # <-- ADD "Delisting"
    if is_corp.any():
...
def _build_out_table(consolidated: pd.DataFrame) -> pd.DataFrame:
    ...
    out = pd.DataFrame({
        ...
        "FX_Rate": consolidated.get("FX_Rate"),  # <--- THIS WAS MISSING
        "__year": pd.to_datetime(consolidated["Date"]).dt.year,
    })

    # Copy internal helper columns if they exist
...
def parse_degiros_csv(df_raw: pd.DataFrame) -> pd.DataFrame:
    df_raw["ISIN"] = df_raw["ISIN"].astype(str).str.strip() # <-- ADD THIS LINE
...
def build_cgt1_export(out: pd.DataFrame, split_audit_df: Optional[pd.DataFrame] = None) -> pd.DataFrame:
    ...
    cols = [
        "CGT Period", "Date Acquired", "Date Disposed", "Ticker - Name", "Asset Type",
        "ISIN", "Quantity", "Sell Proceeds (EUR)", "Gain/Loss (EUR)", "Buys + Fees (EUR)",
    ...
    cgt_period = np.where(
        is_latest,
        bucket_latest,
        year_series.astype("Int64").astype("string")
    )
    ...
    try:
        ...
        _kw = ("ishares","vanguard","vaneck","xtrackers","spdr","lyxor","invesco",
               "wisdomtree","amundi","first trust","global x","ubs","hsbc","etf")
    ...
    if split_audit_df is not None and not split_audit_df.empty:
        ...
        possible_buy_dates = [c for c in sad.columns if ("buy" in c.lower() and "date" in c.lower()) or c.lower() in {"lot_date","buy_date","acq_date"}]
...
def build_output(df_norm: pd.DataFrame, opening_lots: Optional[pd.DataFrame]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if _missing_qty.any():
        _qty_fallback = (
            df.loc[_missing_qty, descr_col]
            .astype(str)
            .str.extract(r'\b(?:buy|sell)\s+(\d+(?:\.\d+)?)', flags=re.I)[0]
        )
            ...
            if t == "Buy":
                ...
                if np.isnan(unit_cost) or unit_cost == 0.0:
                    tot_row = pd.to_numeric(
                        r.get("Total (EUR, fee-adj)", r.get("Total (EUR)", np.nan)),
                        errors="coerce"
                    )
...
def build_form12_export(out: pd.DataFrame, exit_tax_rate: float = 0.41) -> pd.DataFrame:
    ...
    cols = [
        "Tax Year", "Date", "Chargeable Event", "Ticker - Name", "ISIN", "Asset",
        "Quantity", "Proceeds (EUR)", "Cost (EUR)", "Gain/Loss (EUR)",
        "Taxable Gain (EUR)", f"Tax @ {int(exit_tax_rate*100)}% (EUR)",
        "Order ID", "Broker", "Source File"
    ]
...
def _normalize_manual(opening_lots_df):
    """Clean and normalize the manual upload file (Robust Version)."""
    import numpy as np

    if opening_lots_df is None or opening_lots_df.empty:
        return pd.DataFrame(columns=["ISIN","Date","Quantity","EUR_Value","Unit_EUR"])
    ...
    # 1. Date (Crucial: sets Buy date for historical FIFO)
    if "Date" in cols:
        ol["Date"] = pd.to_datetime(ol["Date"], dayfirst=True, errors="coerce")
    else:
        ol["Date"] = pd.Timestamp("2015-01-01") 

    # 2. ISIN Cleanup (Crucial for matching)
    if "ISIN" in ol.columns:
        ol["ISIN"] = ol["ISIN"].astype(str).str.strip()

    # 3. EUR Value (Fixing space vs underscore)
    if "EUR Value" in cols and "EUR_Value" not in cols:
        ol = ol.rename(columns={"EUR Value": "EUR_Value"})
    elif "Value" in cols and "EUR_Value" not in cols:
        ol = ol.rename(columns={"Value": "EUR_Value"})
        
    # Check for required columns
    if {"ISIN", "Quantity", "EUR_Value"}.issubset(ol.columns):
        ol["Quantity"]  = _clean_num(ol["Quantity"])
        ol["EUR_Value"] = _clean_num(ol["EUR_Value"])
        
        ol = ol.dropna(subset=["ISIN", "Quantity", "EUR_Value"])
        ol["Unit_EUR"] = np.where(ol["Quantity"].abs() > 0, ol["EUR_Value"] / ol["Quantity"].abs(), np.nan)
        
        return ol[["ISIN", "Date", "Quantity", "EUR_Value", "Unit_EUR"]]
...
if "opening_lots_df" in locals() and opening_lots_df is not None and not opening_lots_df.empty:
    ...
    if "ISIN" not in df_show.columns:
        ...
        display_cols = ["ISIN"]
        if ticker_col: display_cols.append(ticker_col)
        if qty_col: display_cols.append(qty_col)
        if price_col: display_cols.append(price_col)
        if eur_col: display_cols.append(eur_col)
...
# ===================== LOGIC v3: PRIORITIZE DEGIRO TRANSFERS (ROBUST VERSION) =====================
# 1. Find ALL "Incoming Transfer" rows in DEGIRO.
# 2. Force them to be treated as "Buys" (so Quantity is correct).
# 3. If a Manual Lot matches the ISIN, copy the COST from Manual -> DEGIRO.
# 4. NEVER add a Manual Lot if the ISIN already exists in DEGIRO (Prevents double count).

# ===================== LOGIC v10: AGGRESSIVE DELETION OF ALL CONFLICTING ACQUISITIONS (Final Unit Fix) =====================
if not _manual_norm.empty:
    # Ensure dates are typed correctly
    df_norm["Date"] = pd.to_datetime(df_norm["Date"], errors="coerce")
    
    # 1. Identify ISINs that exist in the Manual File
    manual_isins = set(_manual_norm["ISIN"].astype(str).str.strip().unique())

    # CRASH FIX: Generate transaction type for comparison now, since __Type doesn't exist yet
    temp_type = df_norm["Description"].astype(str).apply(parse_type)

    # 2. Identify ALL Acquisition Rows to be DROPPED
    # We target BUYS, INCOMING TRANSFERS, and any existing MANUAL placeholders.
    is_acquisition = temp_type.astype(str).str.contains(r"\b(?:Buy|Transfer|INCOMING)\b", case=False, na=False)
    
    # Identify sales (to be excluded from dropping)
    is_sale = temp_type.astype(str).str.contains(r"\b(?:Sell|Disposal)\b", case=False, na=False)

    # 3. Define the Drop Mask: [Is an Acquisition OR is an Old Manual ID] AND [Is one of our ISINs] AND [Is NOT a Sale]
    
    # Identify old manual placeholders by Order ID (OPENING- or MANUAL-)
    is_old_manual_id = df_norm["Order ID"].astype(str).str.contains(r"^(OPENING|MANUAL)-", case=False, na=False)

    to_drop = (is_acquisition | is_old_manual_id) & df_norm["ISIN"].astype(str).str.strip().isin(manual_isins) & ~is_sale

    if to_drop.any():
        df_norm = df_norm[~to_drop].copy()

    # 4. ADD Manual Rows with their ORIGINAL DATES (This is the one true Buy lot)
    new_rows = []
    for _, lot in _manual_norm.iterrows():
        isin = str(lot.get("ISIN", "")).strip()
        qty  = float(lot.get("Quantity") or 0.0)
        cost = float(lot.get("EUR_Value") or 0.0)
        unit = float(lot.get("Unit_EUR") or 0.0)
        
        dt = lot.get("Date")
        if pd.isna(dt): dt = pd.Timestamp("2015-01-01")

        if qty > 0:
            new_rows.append({
                "Date": dt,
                "Time": "00:00", 
                "Product": f"Manual {isin}", 
                "ISIN": isin,
                "Description": f"Buy {qty} Manual Import @{unit:.2f} EUR",
                "Change": -cost, 
                "Cash Movements": -cost,
                "__cash_eur": -cost,
                "Order ID": "MANUAL-OPEN", 
                "Type": "Buy", 
                "Currency": "EUR",
                "Quantity": qty, 
                "Price": unit,
                "__Broker": "MANUAL",
                "__SourceFile": "Manual Upload"
            })

    if new_rows:
        df_norm = pd.concat([df_norm, pd.DataFrame(new_rows)], ignore_index=True)
    
    # 5. Sort by Date
    df_norm = df_norm.sort_values(["Date", "Time"], kind="stable").reset_index(drop=True)

# --------- Annual Summary (TOP with tabs) ---------

# --- CGT1: Year filter, preview & download ---
with st.expander("📄 CGT1 export", expanded=False):
        # Build dataset (MUST happen before any UI that uses `out` / `split_audit_df`)
    try:
    ...
    # Build the full export first (unfiltered)
    ...
    if cgt1_df_full.empty:
        ...
        # Friendly sort inside the selection
        ...
        st.dataframe(cgt1_preview)

        # CSV download
        csv_bytes = cgt1_df.to_csv(index=False).encode("utf-8")
        _to_csv = cgt1_df.copy()
        for _col in ["CGT Period", "Date Acquired", "Date Disposed"]:
            if _col in _to_csv.columns:
                _to_csv[_col] = _to_csv[_col].astype("string")
...
def build_summary(df_source: pd.DataFrame, asset_filter: Optional[str]) -> pd.DataFrame:
    ...
    for yr in years_sorted:
        ...
        if asset_filter == "share":
            ...
            realised_gains  = float(_gl_series.clip(lower=0).sum())
            realised_losses = float((-_gl_series.clip(upper=0)).sum())
...
if "out" in locals() and isinstance(out, pd.DataFrame) and not out.empty:
    ...
    if name_col and "ISIN" in out.columns:
        ins = (
            out.loc[:, ["ISIN", name_col]]
               .dropna(subset=["ISIN", name_col])
               .astype({"ISIN": str, name_col: str})
        )
...
if isinstance(out, pd.DataFrame) and not out.empty:
    ...
    incoming_mask = (
        _tmp["Type"].str.contains(r"\b(?:incoming|transfer\s*in|inbound)\b", case=False, na=False) |
        ...
        if isinstance(_manual_norm, pd.DataFrame) and not _manual_norm.empty:
            ...
            manual_by_isin = (
                man.groupby("ISIN", dropna=False)
                   .agg(ManualQty=("Quantity","sum"),
                        ManualEUR=("EUR_Value","sum"))
                   .reset_index()
            )
        ...
        if not lots_map:
            ...
            for isin, lots in lots_map.items():
                ...
                total_cost_eur = float(
                    sum(
                        float(L.get("qty", 0.0)) * float(L.get("unit_cost_eur", 0.0))
                        for L in lots
                    )
                )
                ...
                ccys = {str(L.get("ccy", "") or "").upper() for L in lots}
                ccys.discard("")  # remove blanks
                ...
                if ccys and len(ccys) == 1:
                    ...
                    total_cost_native = float(
                        sum(
                            float(L.get("qty", 0.0))
                            * float(L.get("unit_cost_native", 0.0))
                            for L in lots
                        )
                    )
                    avg_cost_native = (
                        total_cost_native / qty if qty > 0 else np.nan
                    )
            ...
            if not rows:
                ...
                if (
                    "Avg Cost / Unit (Native)" in view.columns
                    and view["Avg Cost / Unit (Native)"].replace("", np.nan).isna().all()
                ):
                ...
                money_cols_eur = [
                    c for c in ["Avg Cost / Unit (EUR)", "Total Cost (EUR)"]
                ...
                invested = float(
                    pd.to_numeric(view["Total Cost (EUR)"], errors="coerce")
                      .fillna(0)
                      .sum()
                )
                st.caption(
    ...
    if not lots_map:
        ...
        if not holding_rows:
            ...
            if qty <= 0 or price_eur <= 0:
                ...
                if qty > avail + 1e-9:
                    st.warning(f"Selected quantity exceeds current holding ({avail:.6f}). Simulation uses available lots for cost basis; consider lowering 'Units to sell'.")
                ...
                if kind == "share":
                    ...
                    tax_now, tax_new, delta = _tax_etf_delta(
                        ytd_gl_now=etfs_ytd_gl,
                        hypo_gl=hypo_gl,
                        rate=exit_tax_rate_etf
                    )
