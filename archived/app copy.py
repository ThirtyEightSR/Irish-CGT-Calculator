...
# datetime module alias removed (unused)
import numpy as np
import pandas as pd
...
# --------------------------------------------------
# 🔍 Dividend Debug Panel
# --------------------------------------------------
with st.expander("🐞 Dividend Debug Panel", expanded=False):
    st.markdown("### 📄 Columns in `out` DataFrame")
    st.write(list(out.columns))
    # Show raw dividend & FX rows
    div_mask = out["Description"].isin(["Dividend", "Dividend Tax"])
    fx_mask  = out["Description"].str.contains("FX", na=False)
    st.markdown("### 💵 Raw Dividend Rows")
    st.dataframe(out[div_mask], use_container_width=True)
    st.markdown("### 💱 Raw FX Rows")
    st.dataframe(out[fx_mask], use_container_width=True)
    # Key columns detected
    value_date_candidates = ["Value date", "Value_date", "ValueDate"]
    change_candidates = ["Change", "Amount", "Variation"]
    val_col = next((c for c in value_date_candidates if c in out.columns), None)
    chg_col = next((c for c in change_candidates if c in out.columns), None)
    st.markdown("### 🏷 Detected Key Columns")
    st.write({"Value Date Column": val_col, "Change Column": chg_col})
    if val_col is None or chg_col is None:
        st.error("❌ Required columns not found. Dividend computation cannot proceed.")
    # Display each dividend group for manual inspection
    if val_col and chg_col:
        grouped = out[div_mask].groupby([val_col, "ISIN"], dropna=False)
        st.markdown("### 📦 Dividend Groups (by Value Date & ISIN)")
        for (vdate, isin), grp in grouped:
            st.markdown(f"**Group: {vdate.date()} – {isin or '(No ISIN)'}**")
            st.dataframe(grp, use_container_width=True)
            native_div = grp[grp["Description"] == "Dividend"][chg_col].sum()
            native_wht = -grp[grp["Description"] == "Dividend Tax"][chg_col].sum()
            native_net = native_div - native_wht
            st.write({"Native Dividend": native_div, "Native WHT": native_wht, "Native Net": native_net})
...
if divs.empty:
    ...
    # Gross and Tax in EUR — directly from `Total` and `Fee`
    divs["Gross_EUR"] = pd.to_numeric(divs["Total"], errors="coerce").fillna(0.0).abs()
    divs["WHT_EUR"] = pd.to_numeric(divs["Fee"], errors="coerce").fillna(0.0)
    divs["WHT_EUR"] = divs["WHT_EUR"].apply(lambda x: -x if x < 0 else x)
    divs["Net_EUR"] = divs["Gross_EUR"] - divs["WHT_EUR"]

    # Yearly summary
    summary = divs.groupby("Year").agg(
