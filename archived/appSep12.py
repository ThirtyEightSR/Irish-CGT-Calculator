...
with st.sidebar:
    ...
    broker_choice = st.selectbox(
        "Broker",
        options=["Auto-detect", "DEGIRO", "Interactive Brokers (CSV – beta)"],
        index=0,
        help="Pick a broker or let the app detect it from headers."
    )
    ...
    with st.expander("📥 Manual Import", expanded=False):
...
def _find_between(df: pd.DataFrame, left: str, right: str) -> str:
    cols = list(df.columns)
    li = cols.index(left); ri = cols.index(right)
    if ri - li != 2:
        candidates = cols[li+1:ri]
        if not candidates:
            raise KeyError(f"No column between {left} and {right}.")
        return candidates[0]
    return cols[li+1]
...
def infer_asset(product, description) -> str:
    def _has_tokens(x) -> bool:
        if not isinstance(x, str):
            return False
        lx = x.lower()
        return any(tok in lx for tok in ETF_TOKENS)
...
def _apply_corporate_actions_and_map_fx(base: pd.DataFrame, grouped: pd.DataFrame, opening_lots: Optional[pd.DataFrame]) -> Tuple[pd.DataFrame, List[Dict]]:
    ...
    # grouped.drop(columns=["__OrderAbsCash", "__Fee_by_oid"], inplace=True, errors="ignore")
    ...
    total_eur_trade = np.where(
        grouped["Type"].eq("Buy"),
        np.where(~pd.isna(order_buy_eur), order_buy_eur, minute_buy_eur),
        np.where(
            grouped["Type"].eq("Sell"),
            np.where(~pd.isna(order_sell_eur), order_sell_eur, minute_sell_eur),
            np.nan
        )
    )
    ...
    # Sort
    type_sort = {"Buy":0,"Sell":1,"Dividend":2,"Stock split":3,"Product change":3,"ISIN change":3,"Coupon":4,"Interest":4,"Fee":5,"Other":6}
    con["__type_sort"] = con["Type"].map(type_sort).fillna(6).astype(int)
    grouped = con
    grouped = con
...
def _consolidate_fifo(grouped: pd.DataFrame) -> pd.DataFrame:
    consolidated = grouped.sort_values(["ISIN","Date","__type_sort","Order ID"], kind="mergesort").reset_index(drop=True)
...
# ---------- Broker adapters (ingest layer) ----------

def detect_broker_from_headers(df_head: pd.DataFrame) -> str:
    """
    Naive detector. Returns one of: 'DEGIRO', 'IB', 'UNKNOWN'
    """
    cols = {c.strip().lower() for c in df_head.columns}

    # Very light heuristics — safe defaults
    degiro_signals = {"date", "product", "isin", "description", "change", "balance"}
    ib_signals = {"asset category", "symbol", "quantity", "multiplier", "trade date", "currency"}

    if degiro_signals <= cols:
        return "DEGIRO"
    if len(ib_signals & cols) >= 3:
        return "IB"
    return "UNKNOWN"


def parse_degiros_csv(df_norm: pd.DataFrame) -> pd.DataFrame:
    """
    Current / existing path:
    - Canonicalize DEGIRO headers and return a DEGIRO-shaped DataFrame
      that your existing pipeline expects.
    """
    df_norm = _canonicalize_headers(df_norm)
    _validate_required_columns(df_norm)
    return df_norm


def parse_ib_csv_minimal(df_norm: pd.DataFrame) -> pd.DataFrame:
    """
    Minimal scaffold for Interactive Brokers CSV (trade log).
    Goal: return a DEGIRO-shaped DataFrame with the columns your pipeline expects:
      Date, Time (optional), Value date (optional), Product, ISIN (blank if unknown),
      Description, FX (optional), Change, Balance, Order ID (optional), Currency (optional)

    This is intentionally conservative (beta). It maps obvious fields
    and leaves ISIN blank unless present. You can evolve this safely.
    """
    df = df_norm.copy()
    # Normalize column names once
    df.columns = [str(c).strip() for c in df.columns]

    # IB exports vary; try a few common names
    # Trade date/time
    if "Trade Date" in df.columns:
        df["Date"] = pd.to_datetime(df["Trade Date"], errors="coerce").dt.strftime("%d-%m-%Y")
    ...
    # ISIN if present in some exports (often not)
    if "ISIN" not in df.columns:
        df["ISIN"] = ""

    # Narrative (build a light description like "Buy 10 XYZ @ 12.34 USD")
    qty_col = "Quantity" if "Quantity" in df.columns else None
    px_col  = "Price"    if "Price" in df.columns    else None
    ccy_col = "Currency" if "Currency" in df.columns else None
    side_col= "Buy/Sell" if "Buy/Sell" in df.columns else None

    def _desc_row(r):
        side = (r.get(side_col) or "").title() if side_col else ""
        qty  = r.get(qty_col)
        px   = r.get(px_col)
        ccy  = (r.get(ccy_col) or "").upper()
        p    = r.get("Product") or ""
        if side and qty is not None and px is not None and ccy:
            return f"{side} {qty} {p}@{px} {ccy}"
        return p

    df["Description"] = df.apply(_desc_row, axis=1)

    # Cash impact / balance surrogate:
    # For now, IB amounts can be in 'Amount' or computed as qty*price with sign from side.
    if "Amount" in df.columns:
        df["Change"] = pd.to_numeric(df["Amount"], errors="coerce")
    else:
        amt = pd.to_numeric(df.get(qty_col, np.nan), errors="coerce") * pd.to_numeric(df.get(px_col, np.nan), errors="coerce")
        sign = np.where(df.get(side_col, "").astype(str).str.lower().str.startswith("sell"), +1,
                np.where(df.get(side_col, "").astype(str).str.lower().str.startswith("buy"), -1, np.nan))
        df["Change"] = amt * sign

    # Balance is not directly present; use a placeholder column so your validator passes
    df["Balance"] = np.nan

    # Optional fields used by your pipeline
    if "Order ID" not in df.columns:
        df["Order ID"] = ""
    if "FX" not in df.columns and "Exchange Rate" in df.columns:
        df["FX"] = df["Exchange Rate"]
    if "Currency" not in df.columns and ccy_col:
        df["Currency"] = df[ccy_col]

    # Keep only columns your pipeline expects (others retained safely)
    keep = ["Date","Product","ISIN","Description","Change","Balance","Order ID","Currency","FX"]
    for k in keep:
...
def build_output(df_norm: pd.DataFrame, opening_lots: Optional[pd.DataFrame]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def _parse_dt(row):
        if has_time:
            dt_str = f"{str(row[date_col]).strip()} {str(row[time_col]).strip()}"
            fmts = ("%d-%m-%Y %H:%M","%d/%m/%Y %H:%M","%Y-%m-%d %H:%M","%d.%m.%Y %H:%M",
                    "%d-%m-%Y %H:%M:%S","%Y-%m-%d %H:%M:%S")
        else:
            dt_str = f"{str(row[date_col]).strip()}"
    ...
    base = pd.DataFrame({
        ...
        "_CashValue": pd.to_numeric(df[between_cash_col], errors="coerce"),
        "Order ID": order_ids,
        "__Type": df["__Type"],
    ...
    # Drop MMF / cash sweep rows (trades + price-change helpers)
...
# Read CSV (robust) + normalize via broker adapters
import traceback
...
try:
    ...
    # Decide broker
    auto_detected = detect_broker_from_headers(df_norm.head(1))
    chosen = broker_choice
    if broker_choice == "Auto-detect":
        chosen = "DEGIRO" if auto_detected in ("DEGIRO", "UNKNOWN") else "Interactive Brokers (CSV – beta)"

    # Dispatch to adapter → returns a DEGIRO-shaped DataFrame the pipeline expects
    if chosen == "DEGIRO":
        df_norm = parse_degiros_csv(df_norm)
        broker_label = "DEGIRO"
    elif chosen.startswith("Interactive Brokers"):
        df_norm = parse_ib_csv_minimal(df_norm)
        # Re-use your existing canonicalization for safety (keeps aliases etc.)
        df_norm = _canonicalize_headers(df_norm)
        _validate_required_columns(df_norm)  # may raise if truly missing
        broker_label = "Interactive Brokers"
    else:
        # Safe default
        df_norm = parse_degiros_csv(df_norm)
        broker_label = "DEGIRO"
...
# (Optional) show what we detected after normalization
st.caption("Detected (normalized) columns:")
...
try:
    ...
    # ---- tag rows with broker & source file so filters work ----
    if "__Broker" not in out.columns:
        out["__Broker"] = "DEGIRO"

    # Streamlit's uploader has .name for filename; fall back if missing
    src_name = getattr(uploaded, "name", None) or "uploaded.csv"
    if "__SourceFile" not in out.columns:
        out["__SourceFile"] = src_name
except Exception as e:
    st.error(f"Could not parse CSV: {e}")
    st.stop()
...
with tabs[4]:
    ...
    if uploaded is not None and 'out' in locals() and out is not None and not out.empty:
