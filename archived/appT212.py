...
def _find_between(df: pd.DataFrame, left: str, right: str) -> str:
    ...
    try:
        ...
        # If either column is missing, fall back to 'left' (pipeline can use Change)
        return left
    if ri - li != 2:
...
def parse_degiros_csv(df_raw: pd.DataFrame) -> pd.DataFrame:
    """
    DEGIRO adapter:
    - Canonicalizes headers
    - Validates required columns
    - Returns the canonical dataframe the pipeline already expects
    """
...
try:
    ...
    # Auto-detect broker and dispatch to its adapter (currently only DEGIRO)
    broker_key = detect_broker_from_headers(df_raw.head(1))
    adapter = BROKER_ADAPTERS.get(broker_key, parse_degiros_csv)
    df_norm = adapter(df_raw)
    detected_label = broker_key  # "DEGIRO" for now
...
with st.expander("Debug: broker ingest (temporary)"):
    st.write("Detected broker:", broker_key)
    st.write("Adapter output shape:", df_norm.shape)
