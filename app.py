...
def _carry_forward_shares_to_year(out_df: pd.DataFrame, year: int, use_exemption: bool, exemption_val: float) -> float:
    ...
    for yr in sorted(k for k in y.keys() if pd.notna(k) and k < year):
        ...
        if realised >= 0:
            ...
            # remaining_gain not required for carry computation
            # remaining_gain = realised - used
            # exemption usage not needed here for carry calculation
...
def _replay_fifo_lots_all(out_df: pd.DataFrame) -> Dict[str, List[Dict]]:
    ...
    if mask_q_from_tot.any():
        qty.loc[mask_q_from_tot] = (tot_eur.loc[mask_q_from_tot].abs() /
                                    price_eur.loc[mask_q_from_tot]).round(6)
    ...
    # Where still NaN but we have total+qty: compute from fee-adjusted total
    mask_uc = unit_cost.isna() & tot_eur.notna() & df["__qty_for_fifo"].gt(0)
    if mask_uc.any():
        unit_cost[mask_uc] = (
            tot_eur[mask_uc].abs() / df.loc[mask_uc, "__qty_for_fifo"]
        ).values
    ...
    for isin, g in df.groupby("ISIN", sort=False):
        ...
        for _, r in g.iterrows():
            ...
            if q <= 1e-12:
                continue
            if t == "Buy":
                # EUR unit cost you already compute
                uc_eur = float(r["__unit_cost"]) if pd.notna(r["__unit_cost"]) else 0.0

                # Native unit price (BEP-style): EUR unit / FX rate when available
                fx = pd.to_numeric(r.get("FX_Rate"), errors="coerce")
                unit_native = np.nan
                if pd.notna(fx) and fx > 0:
                    unit_native = uc_eur / fx

                # Best-effort trade currency label
...
if not _manual_norm.empty and {"ISIN","Date","Product","Description"}.issubset(df_norm.columns):
    ...
    first_product_by_isin = (
        df_norm.dropna(subset=["ISIN"])
               .groupby(df_norm["ISIN"].astype(str))["Product"]
               .first()
               .to_dict()
    )
...
if not _manual_norm.empty and {"ISIN","Description"}.issubset(df_norm.columns):
    ...
    if inc_mask.any():
        ...
        for i, r in df_norm.loc[inc_mask].iterrows():
            ...
            unit_eur = float(pick["Unit_EUR"]) if pd.notna(pick["Unit_EUR"]) else (
                float(pick["EUR_Value"]) / float(pick["Quantity"]) if (pd.notna(pick["EUR_Value"]) and pd.notna(pick["Quantity"]) and pick["Quantity"] != 0) else np.nan
            )
            if pd.isna(unit_eur) or unit_eur <= 0:
                continue
